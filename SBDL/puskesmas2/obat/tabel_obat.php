	...
	<table cellpadding="5" cellspacing="0" border="1">
		...
		if (mysqli_num_rows($query) == 0) {	//ini artinya jika data hasil query di atas kosong
			...
			while ($data = mysqli_fetch_assoc($query)) {	//perulangan while dg membuat variabel $data yang akan mengambil data di database
				...
				echo '<td>' . $data['kegunaan'] . '</td>';
				echo '<td>' . $data['stok'] . '</td>';
				echo '<td><a href="edit_obat.php?id=' . $data['id_obat'] . '">Edit</a> / <a href="hapus.php?id=' . $data['id_obat'] . '" onclick="return confirm(\'Yakin?\')">Hapus</a></td>';	//menampilkan link edit dan hapus dimana tiap link terdapat GET id
				echo '</tr>';
			}
		}
		?>
